...
const EditCreator = () => {
    ...
    const editCreator = async (event) => {
        ...
        .from('creators')
        .update({name: creator.name, imageURL: creator.imageURL, description: creator.description, youtubeURL: creator.youtubeURL, twitterURL: creator.twitterURL, instagramURL: creator.instagramURL})
        .eq('id', id);

        if (error) {
            onsole.error('Error editing row:', error);
        } else {
